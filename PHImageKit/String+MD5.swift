...
extension Int {
    ...
    func ik_bytes(totalBytes: Int = sizeof(Int)) -> [UInt8] {
...
extension NSMutableData {
    ...
    func ik_appendBytes(arrayOfBytes: [UInt8]) {
...
class HashBase {
    ...
    func prepare(len: Int = 64) -> NSMutableData {
        ...
        tmpMessage.ik_appendBytes([0x80]) // append one bit (UInt8 with one bit) to message
...
class MD5 : HashBase {
    ...
    func calculate() -> NSData {
        ...
        let lengthBytes = lengthInBits.ik_bytes(64 / 8)
        tmpMessage.ik_appendBytes(Array(lengthBytes.reverse()));
